...
#Controle dos carros
class Car():
    def __init__(self,speed,posStart,parent) -> None:   
        #Inicializar Carro
        self.windows = pygame.display.get_surface()
        self.carColor = (0,17,255)
        ...
        self.neuronController = neuron.NeuronController(parent.BetterWeights)
    ...
    def checkCrash(self):
        ...
        if(id==1):
            ...
            if(ct.isWin == False):
                ...
                self.parent.BetterWeights.setNewBetterWeights(self)
    ...
    def checkSpeed(self):
        ...
        id = ct.mapId[self.pos]
        if(id == 4):
           self.speedInit += 1 if self.speedInit < 15 else 15

        if(id == 5):
           self.speedInit -= 2 if self.speedInit >  8 else 1 
    ...
    def rendeizeCarWhenInPauser(self):
        ...
        if(self.isWin):
            ct.isWin = True
            self.carRenderized = pygame.draw.circle(self.windows,(0,0,255),self.pos,ct.scaleCar)
        else:
            if(self.isCollider):
                self.carRenderized = pygame.draw.circle(self.windows,(0,0,0),self.pos,ct.scaleCar)
            else:
                self.carRenderized = pygame.draw.circle(self.windows,(100,0,0),self.pos,ct.scaleCar)
    ...
    def move(self):
        ...
        for ss in range(self.speedInit):
            ...
            if self.neuronController.neuroSpeed.valueFinal > self.valueActivetion and self.speedInit < 15:
                self.speedInit += 1
            if self.neuronController.neuroSpeed.valueFinal < self.valueActivetion and self.speedInit > 8:
                self.speedInit -= 1   

            if(isMove and (self.pos not in self.listPosWalked)):
                self.countLimitStop = 0
