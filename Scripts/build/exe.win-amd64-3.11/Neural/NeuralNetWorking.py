...
class NeuralNetWorking():
    ...
    def createCars(self,amountCars,isFirt = True):
        ...
        self.createCarsWithSetValue(amountCars,isFirt)
        ct.theFirstCar = self.listCars[0]

    def createCarsWithSetValue(self,amountCars,isFirt):
        #Criar Lista de Carros no Cenario
        for cars in range(amountCars):   
            self.listCars.append(car.Car(10,(ct.StartPos[0] + 50,ct.StartPos[1] + 50),isFirt,self))
    ...
    def checkInOnlyCar(self,cars):
        ...
        if(((ct.theBest > cars.distanceFromGoal) or cars.isWin or (cars.isWin and ct.theBest > cars.distanceFromGoal))):           
    ...
    def inRunTime(self):
        ...
        valueListCars = ct.maxPopulationInOnTime -  len(self.listCars)
        if((lenCarsDead<ct.population and valueListCars < ct.maxPopulationInOnTime and self.BetterWeights.isWithValues)):
            self.checkFinalBestCar()
            self.createCarsWithSetValue(valueListCars,False)
